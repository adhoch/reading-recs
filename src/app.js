...
/* Three ways in, because one is never enough for six hundred books:
...
/* Author and series stats come from every rated book — the Goodreads export
   merged with every rating logged since — including volumes that have no
   record of their own in books.json. */
...
/* ---------- find a book ---------- */
/* Six hundred nodes is past the point where you can spot a title by eye, so the graph
...
// a view saved by an older build may no longer exist
setView(["mood","graph","shelf","list"].includes(wanted)?wanted:"mood");
if(selected!==null)renderDetail();
